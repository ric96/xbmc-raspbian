...
echo "Installing Build Tools Done"
echo "Installing Dependencies"
sudo apt-get install -y autotools-dev comerr-dev dpkg-dev libalsaplayer-dev \
...
echo "Creating Symbolic Link for libEGL & libGLESv2"
...
echo "Creating Symbolic Link for libEGL & libGLESv2 Done"
echo "Downloading Latest XBMC from Git"
git clone --depth 1 git://github.com/xbmc/xbmc-rbp.git
echo "Download Compleated"
echo "Preparing the XBMC code for compilation"
cd xbmc-rbp/
...
echo "Preparing the XBMC code for compilation Done"
echo "Building XBMC Build Tools"
make -C tools/rbp/depends/xbmc/
echo "Building XBMC Build Tools Done"
...
echo "Fixing Stuff Up"
sed -i 's/-msse2//' lib/libsquish/Makefile
sed -i 's/-DSQUISH_USE_SSE=2//' lib/libsquish/Makefile
echo "Fixing Done"
echo "Making"
make
echo "Make Compleated"
echo "Installing Xbmc"
sudo make install
cd
cd
sudo chmod +x xbmc-update.sh
sudo chmod +x xbmc
echo "Installation Compleated"
echo "Please set gpu memory to 128mb from raspi-config and reboot. Use ./xbmc to start xbmc"
